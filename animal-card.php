...
if (isset($_POST['wishlist_submit'])) {
    ...
    $wish_pub_id = isset($_GET['pub_id']) ? $_GET['pub_id'] : null;
   

    if ($user_id) {
        // Check if the item is already in the wishlist
        $check_query = "SELECT * FROM wishlist WHERE wish_list_id = $user_id AND wish_pub_id = $wish_pub_id";
        $result = mysqli_query($conn, $check_query);

        if (mysqli_num_rows($result) > 0) {
            // If it's in the wishlist, remove it
            $delete_query = "DELETE FROM wishlist WHERE wish_list_id = $user_id AND wish_pub_id = $wish_pub_id";
            mysqli_query($conn, $delete_query);
            $_SESSION['wishlist_message'] = "removed from wishlist .";
        } else {
            // If it's not in the wishlist, add it
            $insert_query = "INSERT INTO wishlist (wish_list_id, wish_pub_id) VALUES ($user_id, $wish_pub_id)";
...
if (isset($_POST['adopt_submit'])) {
    ...
    if ($user_id) {
        // Check if the item is already in the wishlist
        $check_query = "SELECT * FROM adoption WHERE user_id_form = $user_id AND pub_id = $pub_id";
        $result = mysqli_query($conn, $check_query);
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $insert_query = "INSERT INTO adoption (user_id_form, pub_id) VALUES ($user_id, $pub_id)";
            mysqli_query($conn, $insert_query);
...
if ($pub_id) {
    ...
    $sql = "SELECT * FROM pub_adoption WHERE pub_id = '$pub_id'";
    $result = $conn->query($sql);

    if ($result && $result->num_rows > 0) {
        $pub_adoption = $result->fetch_assoc();
        $user_id = $pub_adoption['user_id_pub'];
        ...
        if ($user_result && $user_result->num_rows > 0) {
            ...
            echo '<img src="' . $pub_adoption['anim_image1'] . '" width="50%" class="mainimg" alt="">';
            echo '<div class="small-img-groupe">';
            echo '<div class="small-img-col">';
            echo '<img src="' . $pub_adoption['anim_image1'] . '" width="100%" class="small-img" alt="">';
            echo '</div>';
            echo '<div class="small-img-col">';
            echo '<img src="' . $pub_adoption['anim_image2'] . '" width="100%" class="small-img" alt="">';
            echo '</div>';
            echo '<div class="small-img-col">';
            echo '<img src="' . $pub_adoption['anim_image3'] . '" width="100%" class="small-img" alt="">';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '<div class="single-anim-details">';
            echo '<h6>' . $pub_adoption['anim_type'] . '</h6>';
            echo '<div class="adresse-container"><ion-icon name="pin-outline"></ion-icon><p class="adresse">' . $user['user_city'] . '</p></div>';
            echo '<div class="box-container">';
            echo '<div class="box"> name <br> <strong> ' . $pub_adoption['anim_name'] . '</strong></div>';
            echo '<div class="box">Gender <br> <strong>' . $pub_adoption['anim_gender'] . '</strong></div>';
            echo '<div class="box">Race <br> <strong>' . $pub_adoption['anim_race'] . '</strong></div>';
            echo '<div class="box"> Age <br> <strong>' . $pub_adoption['anim_age'] . '</strong></div>';
            echo '</div>';
            echo '<h4>Pet details</h4>';
            echo '<p class="description">' . $pub_adoption['anim_description'] . '</p>';
            ...
            echo '</a>';
            echo '<a href="https://www.google.com/maps/search/?api=1&query=' . $user['user_address'] . '" class="btn-link">';
            echo '<ion-icon name="arrow-forward" aria-hidden="true"></ion-icon>';
            echo '<span>Get address </span>';
            echo '</a>';
            echo '<div class="btn-container">';
            echo '<form method="post">';
            echo '<button class="btn" name="wishlist_submit">Wishlist <ion-icon name="heart-outline"></ion-icon></button>';
            echo '</form>';
            echo '<form method="post">';
            echo '<button class="btn" name="adopt_submit" >Adopt Now</button>';
            echo '</form>';
            echo '</div>';
            if ($wishlist_message) {
                echo '<span id="massg">' . $wishlist_message . '</span>';
            }
            if ($adopt_message) {
