...
$sql = "SELECT pub_missing.*, member.user_name, member.user_phone, member.user_id, member.user_email, member.user_city, member.user_address, member.user_gender
        FROM pub_missing
        INNER JOIN formulaire ON pub_missing.miss_id = formulaire.miss_id 
        INNER JOIN member ON formulaire.user_id_form = member.user_id
        WHERE pub_missing.user_id = '$user_id'";
