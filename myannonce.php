...
function delete_pub_adoption($conn, $pub_id, $from_wishlist = false) {
    ...
    $delete_query = "DELETE FROM pub_adoption WHERE pub_id = '$pub_id'";
...
if(isset($_POST['delete_submit'])) {
    ...
    $pub_id_to_delete = $_POST['pub_adoption_id'];
...
$sql = "SELECT * FROM pub_adoption WHERE user_id_pub = $user_id ";
$result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo '<input type="hidden" name="pub_adoption_id" value="' . $row['pub_id'] . '">';
            echo '<button name="delete_submit" class="login-btn">Delete</button>';
          
            
             
            echo '<a href="edit-annonce.php?pub_id='.$row['pub_id'].'">Edit</a>';
        ...
        echo "<h3>You have no publication</h3>";
    }
?>
