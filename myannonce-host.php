...
function delete_pub_adoption($conn, $pub_id, $from_wishlist = false) {
    ...
    $delete_query = "DELETE FROM pub_host WHERE host_id = '$pub_id'";
...
if(isset($_POST['delete_submit'])) {
    ...
    $pub_id_to_delete = $_POST['host_id'];
...
$sql = "SELECT * FROM pub_host WHERE user_id = $user_id ";
$result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo '<input type="hidden" name="host_id" value="' . $row['host_id'] . '">';
            echo '<button name="delete_submit" class="login-btn">Delete</button>';
          
            
             
            echo '<a href="edit-annonce-host.php?host_id='.$row['host_id'].'">Edit</a>';
