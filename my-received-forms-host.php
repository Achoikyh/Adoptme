...
$sql = "SELECT pub_host.*, member.user_name, member.user_phone, member.user_id, member.user_email, member.user_city, member.user_address, member.user_gender
       FROM pub_host
        INNER JOIN hosting ON pub_host.host_id = hosting.hosting_pub_id 
        INNER JOIN member ON hosting.hosting_user_id = member.user_id
        WHERE pub_host.user_id = '$user_id'";
